...
appendTable = function(table_name, object, prefix = "", update_id = NA, to_upper = TRUE, user_name = NULL) {
  ...
  getTable(table_name, function(table, con) {
    ...
    if(table %>% filter(ID %in% id) %>% summarize(n=n()) %>% pull() == 0 | !is.na(update_id)){
      ...
      log_success_dev(paste0("Added/updated: ", paste0(id, collapse = ", ")))
      return(id)
    } else {
...
addCustomer = function(FIRST_NAME = NULL, LAST_NAME = NULL, update_id = NA){
  new_object = tibble(
    FIRST_NAME = FIRST_NAME,
    LAST_NAME = LAST_NAME)
  result = appendTable("customers",new_object, prefix = "C", update_id = update_id)
  return(result)
}

addProduct = function(PRODUCT_NAME = NULL, update_id = NA){
  new_object = tibble(
    PRODUCT_NAME = PRODUCT_NAME
  )
  result = appendTable("products",new_object, prefix = "P", update_id = update_id)
  return(result)
}

addTransaction = function(CUSTOMER_ID = NULL,PRODUCT_ID = NULL,TRANSMISSION_ID = NULL,PRODUCT_PRICE = NULL, IS_PAID = NULL, IS_DELIVERED = NULL, update_id = NA){
  new_object = tibble(
  ...
  result = appendTable("transactions",new_object, prefix = "T", update_id = update_id)
  return(result)
}
